...
Partner 1: Xu Zheng     xz233
Partner 2: Hanyu Xie    hx54
Date: 11/19/2018 01:39 am
"""
...
"""
detectArbitrage
implement Bellman-Ford to detect arbitrage according to given currencies
return a path if an arbitrage exists  
input:
    currencies: the Currencies object for the exchange rates
    tol: tolerance value used in distance update
output:
    a list of possible negative cost cycle
"""
def detectArbitrage(currencies, tol=1e-15):
    ##### Your implementation goes here. #####
    # initialize the graph and set the start node
    ...
    # iterate |V|-1 times
    for iter in range(0, len(currencies.adjList)):
        # look at each vertex
        for vertex in currencies.adjList:
    ...
    set = []
    for vertex in currencies.adjList:
        # vertices within a negative circle will be updated a shorter distance
        # vertices being pointed by a neg-circle vertex will also be updated
        for neigh in vertex.neigh:
            if neigh.dist > vertex.dist + currencies.adjMat[vertex.rank][neigh.rank] + tol:
                set.append(neigh)

    # if neg-circle does not exist, return empty list
    if len(set) == 0:
        return []

    # choose a vertex that had a change
    cur = set[0]
    path = []
    # follow its path backwards until a cycle
    while cur.prev not in path:
        path.append(cur)
        cur = cur.prev
    # now vertex cur must be in a neg-cycle
    # remove any vertices that are not part of the cycle
    # by tracing and adding the path backwards
    path.clear()
    while cur.rank not in path:
        path.append(cur.rank)
        cur = cur.prev
    # again add starting vertex into the path
    path.append(cur.rank)
    # reverse path in right order
    path.reverse()
    return path
    ##### Your implementation goes here. #####
...
"""
rates2mat
create adjacency matrix according to exchange rates
input:
    rates matrix
output:
    adjacency matrix
